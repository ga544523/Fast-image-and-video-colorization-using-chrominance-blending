...
Math.Abs(vectory[now.i, now.j] - vectory[now.i , now.j+1]))
            ...
            for (int i = 0; i < row; i++)
            {
                for (int j = 0; j < col; j++)
                {
                    resultimage.Data[i, j, 0] = (Byte)( 1.164*(vectory[i, j]) + 2.018 * (resultcb[i, j] )-276.928);

 resultimage.Data[i, j, 1] = (Byte) (1.164 * (vectory[i,j] ) - 0.391 * (resultcb[i,j] ) - 0.813 * (resultcr[i,j]  )+135.488 );
                    resultimage.Data[i, j, 2] =  (Byte)(1.164*(vectory[i,j] ) + 1.596 * (resultcr[i, j] )-222.912);
